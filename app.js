...
const ANALYTICS_ALLOWED_EMAILS = new Set([
  'admin@hctinstitute.edu.ph',
  'analytics@hctinstitute.edu.ph'
]);
...
function patientTemplate(input) {
  return {
    ...
    carePlan:input.carePlan||[carePlanEmpty()],
...
let saveTimer=null;
let cloud={ready:false,client:null,session:null,profile:null,status:'Local only',busy:false,sync:'offline',queued:0,lastSaved:''};

function loadState(){
   try{const s=JSON.parse(localStorage.getItem(LOCAL_KEY)||'null');if(s&&Array.isArray(s.patients)&&s.patients.length){s.settings=s.settings||{theme:'light',language:'en',interactive:true};s.customScenarios=s.customScenarios||[];s.patients.forEach(p=>{p.peerReviews=p.peerReviews||[];p.facultyModules=p.facultyModules||[];p.interactionLog=p.interactionLog||[];p.carePlan=(p.carePlan||[]).map(c=>({assessment:c.assessment||c.cues||'',dx:c.dx||'',goal:c.goal||'',interventions:c.interventions||'',evaluation:c.evaluation||''}));});return s;}}catch(e){}
  return{patients:[clone(scenarios[0].patient)],activeId:'heg-default',tab:'summary',submitted:{},customScenarios:[],settings:{theme:'light',language:'en',interactive:true}};
}
function currentPatient(){return state.patients.find(p=>p.id===state.activeId)||state.patients[0];}
function scenarioLibrary(){return[...scenarios,...(state.customScenarios||[])];}
function scenarioName(id){return(scenarioLibrary().find(s=>s.id===id)||{}).name||'Student-created case';}
...
function setSync(status,msg){
  cloud.sync=status;
  if(msg)cloud.status=msg;
  cloud.queued=readCloudQueue().length;
  renderCloudOnly();
}
function scheduleCloudSave(){
  if(!cloud.ready||!cloud.session){setSync('offline','Offline - export JSON if bandwidth is low');return;}
  setSync('saving','Saving to Supabase...');
  clearTimeout(saveTimer);
  saveTimer=setTimeout(()=>saveCurrentChart(false),650);
}
function readCloudQueue(){try{return JSON.parse(localStorage.getItem(CLOUD_QUEUE_KEY)||'[]')||[];}catch(e){return[];}}
function writeCloudQueue(q){localStorage.setItem(CLOUD_QUEUE_KEY,JSON.stringify(q.slice(-50)));cloud.queued=q.length;}
function queueCloudRecord(rec,reason){const q=readCloudQueue();q.push({rec,reason:String(reason||'offline'),queuedAt:new Date().toISOString()});writeCloudQueue(q);}
async function flushCloudQueue(){
  if(!cloud.ready||!cloud.session)return;
  const q=readCloudQueue();
  if(!q.length)return;
  setSync('saving','Saving queued charts...');
  const keep=[];
  for(const item of q){
    const{error}=await cloud.client.from('chart_attempts').upsert(item.rec,{onConflict:'user_id,local_patient_id'});
    if(error)keep.push(item);
  }
  writeCloudQueue(keep);
  setSync(keep.length?'offline':'saved',keep.length?`Offline - ${keep.length} chart save(s) queued`:'Saved to Supabase');
}
function userEmail(){return String(cloud.session?.user?.email||cloud.profile?.email||'').toLowerCase().trim();}
function emailDomain(email){return String(email||'').toLowerCase().trim().split('@').pop()||'';}
function allowedRegistrationEmail(email){const d=emailDomain(email);return ALLOWED_REGISTRATION_DOMAINS.some(x=>d===x||d.endsWith('.'+x));}
function privacyConsentId(){return `${PRIVACY_CONSENT_KEY}_${cloud.session?.user?.id||'local'}`;}
function hasPrivacyConsent(){
  if(cloud.profile?.role==='instructor')return true;
  if(cloud.profile?.privacy_consent_at)return true;
  if(cloud.session?.user?.user_metadata?.privacy_consent_at)return true;
  return localStorage.getItem(privacyConsentId())==='accepted';
}
function canAccessAnalytics(){
  const p=cloud.profile||{};
  return p.role==='instructor' && (p.analytics_access===true || p.dashboard_access===true || ANALYTICS_ALLOWED_EMAILS.has(userEmail()));
}
...
function drugMeta(itemOrText){const h=window.HctMedicationInventory;if(!h)return null;if(typeof itemOrText==='object')return h.find(itemOrText.id)||h.find(itemOrText.name);return h.find(itemOrText);}
function drugButton(itemOrText){const m=drugMeta(itemOrText);return m?`<button class="drug-info-btn" data-med-info="${esc(m.id)}" title="Open drug reference sidebar">Reference</button>`:'';}
...
async function initSupabase(){
  ...
  cloud.sync=cloud.session?'saved':'offline';
  if(cloud.session)flushCloudQueue();
  render();
}
async function loadProfile(){
  ...
  const p={id:u.id,email:u.email,full_name:m.full_name||'',student_id:m.student_id||'',section:m.section||'',institution:m.institution||'',role:m.role||'student',privacy_consent_at:m.privacy_consent_at||null,privacy_consent_law:m.privacy_consent_law||null};
...
async function signUp() {
  ...
  const consent = document.getElementById('auth-privacy-consent')?.checked;
 
  if (!e || !pw || !name) { showAuthError('Name, email, and password are required.'); return; }
  if (!allowedRegistrationEmail(e)) {
    showAuthError('Registration is limited to hct.ph and affiliated HCT email domains.');
    return;
  }
  ...
  if (accountType === 'student' && !consent) {
    showAuthError('Please review and accept the Data Privacy Act consent before creating an examinee account.');
    return;
  }
  ...
  if (accountType === 'student') {
    ...
    meta.role        = 'student';
    meta.privacy_consent_at = new Date().toISOString();
    meta.privacy_consent_law = 'RA 10173';
  } else {
  ...
  if (data.session) {
    cloud.session = data.session;
    localStorage.setItem(`${PRIVACY_CONSENT_KEY}_${data.session.user.id}`,'accepted');
...
async function upsertProfile(m={}){if(!cloud.session)return;const u=cloud.session.user;const r=m.role||cloud.profile?.role||'student';const p={id:u.id,email:u.email,full_name:m.full_name||cloud.profile?.full_name||'',student_id:m.student_id||cloud.profile?.student_id||'',section:m.section||cloud.profile?.section||'',institution:m.institution||cloud.profile?.institution||'',role:r,privacy_consent_at:m.privacy_consent_at||cloud.profile?.privacy_consent_at||null,privacy_consent_law:m.privacy_consent_law||cloud.profile?.privacy_consent_law||null};const{error}=await cloud.client.from('profiles').upsert(p,{onConflict:'id'});if(error)toast(error.message);else cloud.profile={...cloud.profile,...p};}
...
async function saveCurrentChart(showToast=true,status='draft'){
  if(!cloud.ready||!cloud.session){setSync('offline','Offline - sign in to save centrally');if(showToast)toast('Sign in to save centrally');return;}
  const rec=chartRecord(status);
  setSync('saving','Saving to Supabase...');
  const{error}=await cloud.client.from('chart_attempts').upsert(rec,{onConflict:'user_id,local_patient_id'});
  if(error){queueCloudRecord(rec,error.message);setSync('offline',`Offline - ${readCloudQueue().length} chart save(s) queued`);if(showToast)toast('Offline - queued for Supabase retry');}
  else{cloud.lastSaved=new Date().toISOString();setSync('saved',rec.status==='submitted'?'Submitted to instructor':'Saved to Supabase');if(showToast)toast(cloud.status);}
  renderCloudOnly();
}
...
async function fetchDashboard(){
  if(!canAccessAnalytics()){toast('Analytics dashboard access is restricted.');return;}
...
function render() {
  ...
  if (!hasPrivacyConsent()) {
    document.getElementById('app').innerHTML = renderPrivacyConsentPage();
    bindPrivacyConsentEvents();
    return;
  }
...
function renderPrivacyConsentPage(){
  return `
      ...
      <div class="auth-form-panel">
        <div class="auth-card privacy-card">
          ...
          <div class="notice info" style="margin:14px 0;">
            ...
            <span>Under the Data Privacy Act of 2012, HCT Institute will collect and process your account details, test responses, scores, timestamps, and related assessment records only after privacy review and for identity verification, test administration, scoring, analytics, academic support, security, and required institutional reporting.</span>
          </div>
          <p class="privacy-copy">HCT must complete a Privacy Impact Assessment before collecting student personally identifiable information and register with the National Privacy Commission when processing sensitive personal data. Registration is restricted to hct.ph and affiliated HCT domains.</p>
          <p class="privacy-copy">Your information will be handled with appropriate safeguards and accessed only by authorized personnel. Draft simulation charts are retained for the active class term plus one academic year unless a longer school or legal requirement applies; deletion, correction, or access requests may be made through the HCT data protection contact or your instructor.</p>
...
function bindPrivacyConsentEvents(){
  document.getElementById('accept-privacy-consent')?.addEventListener('click', async()=>{
    if(!document.getElementById('privacy-consent-check')?.checked){toast('Please accept the Data Privacy consent to continue.');return;}
    const acceptedAt=new Date().toISOString();
    localStorage.setItem(privacyConsentId(),'accepted');
    if(cloud.profile)cloud.profile={...cloud.profile,privacy_consent_at:acceptedAt,privacy_consent_law:'RA 10173'};
    await upsertProfile({privacy_consent_at:acceptedAt,privacy_consent_law:'RA 10173'});
    render();
  });
  document.getElementById('privacy-signout')?.addEventListener('click',signOut);
}
function renderCloudPanel() {
  ...
  const syncLabel = cloud.sync === 'saving' ? '⟳ Saving' : cloud.sync === 'saved' ? '✓ Saved' : '✗ Offline';
  const syncClass = cloud.sync === 'saving' ? 'saving' : cloud.sync === 'saved' ? 'saved' : 'offline';
  ...
  return `
    <section class="cloud-panel" id="cloud-panel">
      <div class="cloud-status">
        ...
        <span class="sync-indicator ${syncClass}">${syncLabel}${cloud.queued?` · ${cloud.queued} queued`:''}</span>
      ...
      <div class="actions">
        ...
        ${canAccessAnalytics()
          ? '<button class="btn" id="dashboard-shortcut">Analytics dashboard</button>'
          : ''}
...
function isFacultyTab(tab){ return FACULTY_TABS.has(tab) && !canAccessAnalytics(); }
function renderNav(){
  const hasAnalytics = canAccessAnalytics();
  const t = state.tab;
  ...
  const ns = (id, label, inner, childTabs=[]) => {
    const open = !!navOpen[id] || childTabs.includes(t);
  ...
  return `<nav class="nav">
    ...
    ${ns('sec-respiratory','Respiratory',nb('resp-assessment','Respiratory Assessment',true)+nb('resp-medications','Respiratory Medications',true)+nb('resp-interventions','Respiratory Interventions',true),['resp-assessment','resp-medications','resp-interventions'])}
    ${ns('sec-obstetrics','Obstetrics','',[])}
    ${nb('problem-list','Problem List')}
    ${ns('sec-patient-history','Patient History','',[])}
    ${nb('sbar','SBAR')}
    ${nb('careplan','Care Plan')}
    ${nb('roys-model',"Roy's Adaptation Model")}
    ${ns('sec-patient-forms','Patient Forms','',[])}
    <div class="group">Learning tools</div>
    ${[['reasoning','CR','Clinical reasoning'],['peerreview','PR','Peer review mode'],['debriefing','DB','Debriefing'],['progress','PT','My progress'],['report','RP','Print report'],['scenarios','SC','Sample scenarios'],['newpatient','NP','Add patient']].map(([tab,ic,label])=>icBtn(tab,ic,label)).join('')}
  ...
  return `
    <div class="auth-shell">
      ...
      <div class="auth-form-panel">
        ...
        <p class="auth-footer">
          HCT Institute · Healthcare and Technology Institute Inc. · Pasig City, Philippines
        </p>
      </div><!-- /auth-form-panel -->
  ...
  return `
    <div class="auth-shell">
      ...
      <div class="auth-form-panel">
        ...
        <p class="auth-footer">HCT Institute · Pasig City, Philippines</p>
      </div>
    </div>`;
}
...
function rSummary(){
  ...
  const syncedMeds=(p.scenarioId==='scratch' ? (p.orders||[]) : []).filter(o=>isMedOrder(o)&&!existingMedNames.has((o.order||'').toLowerCase().trim())).map((o,idx)=>({id:`sync-${idx}`,name:o.order,dose:o.details||'—',route:'',freq:o.status,priority:o.category,status:o.nurseCompleted?'given':'pending',time:'',note:'',warn:'',_fromOrder:true}));
...
// ── Build synced lab entries from orders ──────────────────────────────────
  ...
  const syncedFromOrders = (p.scenarioId === 'scratch' ? (p.orders||[]) : [])
...
function rMeds(){
  ...
  const rows = allMeds.map((m, i) => {
    ...
    return `<tr>
      <td>
        <strong>${esc(m.name)}</strong> ${drugButton(m.name)}${syncTag}<br>
...
function getNoteTemplate(type){
  ...
  const latestMedAction = (p.meds||[])
    .filter(m => ['given','not_given','hold'].includes(m.status))
    .slice()
    .sort((a,b) => String(b.time||'').localeCompare(String(a.time||'')))[0] || {};
  const medStatusLabel = latestMedAction.status === 'not_given'
    ? 'Not given'
    : latestMedAction.status === 'hold'
      ? 'Held'
      : latestMedAction.status === 'given'
        ? 'Given'
        : 'No MAR medication action documented';
  const medDoseRouteFreq = [latestMedAction.dose, latestMedAction.route, latestMedAction.freq].filter(Boolean).join(' / ');
  const fill = t => t
    ...
    .replace(/\{\{medName\}\}/g,     latestMedAction.name||'No MAR medication selected')
    .replace(/\{\{medDoseRouteFreq\}\}/g, medDoseRouteFreq||'--')
    .replace(/\{\{medStatus\}\}/g,   medStatusLabel)
    .replace(/\{\{medTime\}\}/g,     latestMedAction.time ? nowDisplay(latestMedAction.time) : '--')
    .replace(/\{\{medNote\}\}/g,     latestMedAction.note||'No MAR note documented')
    .replace(/\{\{medSafety\}\}/g,   latestMedAction.warn||'No MAR safety note documented')
...
- Drug: {{medName}}
- Dose / Route / Frequency: {{medDoseRouteFreq}}
- MAR status: {{medStatus}}
- Indication:
- Verified: Patient ID [ ]  Allergy [ ]  MAR [ ]  Labs [ ]  Right time [ ]
- MAR safety note: {{medSafety}}
...
- Site / Method:
- Time documented in MAR: {{medTime}}
- MAR note: {{medNote}}
...
function showPatientBand(){
        ...
        <div class="band-card${hasAllergy?' band-card--allergy':''}" id="band-printable">
          ...
          <div class="band-inner">
            ...
            <div class="band-center">
              ...
              <div class="band-facility">HCT Institute</div>
          ...
          <div class="band-bottom-strip">
            ...
            <span>HCT Institute Simulation</span>
          </div>
  ...
  document.getElementById('band-print-btn')?.addEventListener('click',()=>printWithMode('band'));
...
const POCKET_GUIDE_DIAGNOSIS_NAMES=[
  'Activity Intolerance','Risk for Activity Intolerance','Ineffective Activity Planning','Risk for Ineffective Activity Planning','Risk for Disuse Syndrome','Decreased Diversional Activity Engagement','Fatigue','Insomnia','Sedentary Lifestyle','Readiness for Enhanced Sleep','Sleep Deprivation','Disturbed Sleep Pattern','Impaired Transfer Ability','Impaired Walking','Decreased Intracranial Adaptive Capacity','Autonomic Dysreflexia','Risk for Autonomic Dysreflexia','Risk for Bleeding','Risk for Unstable Blood Pressure','Decreased Cardiac Output','Risk for Decreased Cardiac Output','Risk for Ineffective Gastrointestinal Perfusion','Risk for Metabolic Imbalance Syndrome','Risk for Shock','Ineffective Peripheral Tissue Perfusion','Risk for Ineffective Peripheral Tissue Perfusion','Risk for Decreased Cardiac Tissue Perfusion','Risk for Ineffective Cerebral Tissue Perfusion','Anxiety','Disturbed Body Image','Defensive Coping','Ineffective Coping','Readiness for Enhanced Coping','Death Anxiety','Readiness for Enhanced Decision-Making','Decisional Conflict','Ineffective Denial','Impaired Emancipated Decision-Making','Risk for Impaired Emancipated Decision-Making','Readiness for Enhanced Emancipated Decision-Making','Labile Emotional Control','Imbalanced Energy Field','Fear','Grieving','Complicated Grieving','Risk for Complicated Grieving','Readiness for Enhanced Hope','Hopelessness','Risk for Compromised Human Dignity','Ineffective Impulse Control','Impaired Mood Regulation','Moral Distress','Disturbed Personal Identity','Risk for Disturbed Personal Identity','Post-Trauma Syndrome','Risk for Post-Trauma Syndrome','Readiness for Enhanced Power','Powerlessness','Risk for Powerlessness','Rape-Trauma Syndrome','Ineffective Relationship','Risk for Ineffective Relationship','Readiness for Enhanced Relationship','Impaired Religiosity','Risk for Impaired Religiosity','Readiness for Enhanced Religiosity','Relocation Stress Syndrome','Risk for Relocation Stress Syndrome','Impaired Resilience','Risk for Impaired Resilience','Readiness for Enhanced Resilience','Readiness for Enhanced Self-Concept','Chronic Low Self-Esteem','Risk for Chronic Low Self-Esteem','Situational Low Self-Esteem','Risk for Situational Low Self-Esteem','Chronic Sorrow','Spiritual Distress','Risk for Spiritual Distress','Readiness for Enhanced Spiritual Well-Being','Stress Overload','Constipation','Risk for Constipation','Chronic Functional Constipation','Risk for Chronic Functional Constipation','Perceived Constipation','Diarrhea','Impaired Urinary Elimination','Dysfunctional Gastrointestinal Motility','Risk for Dysfunctional Gastrointestinal Motility','Bowel Incontinence','Functional Urinary Incontinence','Overflow Urinary Incontinence','Reflex Urinary Incontinence','Stress Urinary Incontinence','Urge Urinary Incontinence','Risk for Urge Urinary Incontinence','Urinary Retention','Risk for Unstable Blood Glucose Level','Insufficient Breast Milk Production','Ineffective Breastfeeding','Interrupted Breastfeeding','Readiness for Enhanced Breastfeeding','Impaired Dentition','Ineffective Adolescent Eating Dynamics','Ineffective Child Eating Dynamics','Ineffective Infant Eating Dynamics','Risk for Electrolyte Imbalance','Ineffective Infant Feeding Pattern','Deficient Fluid Volume','Excess Fluid Volume','Risk for Deficient Fluid Volume','Risk for Imbalanced Fluid Volume','Risk for Impaired Liver Function','Impaired Oral Mucous Membrane Integrity','Risk for Impaired Oral Mucous Membrane Integrity','Nausea','Imbalanced Nutrition: Less Than Body Requirements','Readiness for Enhanced Nutrition','Obesity','Overweight','Risk for Overweight','Impaired Swallowing','Readiness for Enhanced Self-Care','Self-Care Deficit: Bathing','Self-Care Deficit: Dressing','Self-Care Deficit: Feeding','Self-Care Deficit: Toileting','Self-Neglect','Disorganized Infant Behavior','Risk for Disorganized Infant Behavior','Readiness for Enhanced Organized Infant Behavior','Acute Confusion','Risk for Acute Confusion','Chronic Confusion','Impaired Memory','Risk for Peripheral Neurovascular Dysfunction','Disturbed Sensory Perception','Unilateral Neglect','Impaired Comfort','Readiness for Enhanced Comfort','Acute Pain','Chronic Pain','Chronic Pain Syndrome','Labor Pain','Ineffective Airway Clearance','Risk for Aspiration','Ineffective Breathing Pattern','Impaired Gas Exchange','Impaired Spontaneous Ventilation','Dysfunctional Ventilatory Weaning Response','Acute Substance Withdrawal Syndrome','Risk for Acute Substance Withdrawal Syndrome','Risk for Adverse Reaction to Iodinated Contrast Media','Risk for Allergy Reaction','Contamination','Risk for Contamination','Risk for Dry Eye','Risk for Dry Mouth','Risk for Falls','Risk for Female Genital Mutilation','Frail Elderly Syndrome','Risk for Frail Elderly Syndrome','Ineffective Health Maintenance','Impaired Home Maintenance','Neonatal Hyperbilirubinemia','Risk for Neonatal Hyperbilirubinemia','Hyperthermia','Hypothermia','Risk for Hypothermia','Risk for Perioperative Hypothermia','Risk for Infection','Risk for Injury','Risk for Corneal Injury','Risk for Urinary Tract Injury','Latex Allergy Reaction','Risk for Latex Allergy Reaction','Risk for Disturbed Maternal-Fetal Dyad','Impaired Bed Mobility','Impaired Physical Mobility','Impaired Wheelchair Mobility','Neonatal Abstinence Syndrome','Risk for Occupational Injury','Risk for Poisoning','Risk for Perioperative Positioning Injury','Risk for Pressure Ulcer','Ineffective Protection','Self-Mutilation','Risk for Self-Mutilation','Impaired Sitting','Impaired Skin Integrity','Risk for Impaired Skin Integrity','Impaired Standing','Risk for Sudden Infant Death','Risk for Suffocation','Risk for Suicide','Delayed Surgical Recovery','Risk for Delayed Surgical Recovery','Risk for Surgical Site Infection','Risk for Thermal Injury','Ineffective Thermoregulation','Risk for Ineffective Thermoregulation','Risk for Venous Thromboembolism','Impaired Tissue Integrity','Risk for Impaired Tissue Integrity','Risk for Physical Trauma','Risk for Vascular Trauma','Risk for Other-Directed Violence','Risk for Self-Directed Violence','Wandering','Ineffective Childbearing Process','Risk for Ineffective Childbearing Process','Readiness for Enhanced Childbearing Process','Sexual Dysfunction','Ineffective Sexuality Pattern','Risk for Impaired Attachment','Impaired Verbal Communication','Readiness for Enhanced Communication','Compromised Family Coping','Disabled Family Coping','Ineffective Community Coping','Readiness for Enhanced Community Coping','Readiness for Enhanced Family Coping','Dysfunctional Family Processes','Interrupted Family Processes','Readiness for Enhanced Family Processes','Risk for Complicated Immigration Transition','Risk for Loneliness','Impaired Parenting','Risk for Impaired Parenting','Readiness for Enhanced Parenting','Parental Role Conflict','Ineffective Role Performance','Caregiver Role Strain','Risk for Caregiver Role Strain','Impaired Social Interaction','Social Isolation','Risk for Delayed Development','Risk for Disproportionate Growth','Risk-Prone Health Behavior','Deficient Community Health','Readiness for Enhanced Health Literacy','Ineffective Health Management','Ineffective Family Health Management','Readiness for Enhanced Health Management','Deficient Knowledge','Readiness for Enhanced Knowledge'
];
function careGuideDomain(dx){const s=String(dx).toLowerCase();if(/airway|breath|gas|ventilat|aspirat|oxygen/.test(s))return'respiration';if(/fluid|electrolyte|nutrition|glucose|nausea|diarrhea|constipation|swallow|feeding|liver|mucous|obesity|overweight/.test(s))return'foodfluid';if(/pain|comfort/.test(s))return'pain';if(/infection|injury|fall|trauma|suicide|violence|skin|tissue|thermo|bleed|shock|poison|allergy|latex|suffocation|pressure|protection/.test(s))return'safety';if(/cardiac|perfusion|blood pressure|bleeding|intracranial|neurovascular/.test(s))return'circulation';if(/urinary|bowel|elimination|incontinence|retention/.test(s))return'elimination';if(/communication|family|coping|relationship|parenting|social|loneliness|attachment|role/.test(s))return'social';if(/knowledge|health|development|literacy|growth|behavior/.test(s))return'teaching';if(/anxiety|fear|grieving|esteem|spiritual|moral|power|identity|resilience|trauma syndrome|denial|decision|mood|hope|sorrow|stress/.test(s))return'ego';if(/sleep|activity|fatigue|mobility|walking|standing|sitting|self-care|neglect|transfer/.test(s))return'activity';return'general';}
const CARE_GUIDE_DOMAIN_TEXT={
  general:{sub:['Reports change from baseline','States concern, discomfort, or difficulty performing usual role','Verbalizes need for help or information'],obj:['Abnormal assessment finding linked to diagnosis','New or worsening vital sign, lab, behavior, or functional change','Orders, devices, or risk factors present in chart'],goals:['Priority problem is stabilized or prevented during the shift','Patient/caregiver verbalizes the plan and warning signs before handoff','Findings trend toward baseline with no preventable complication'],ints:['Complete focused assessment, verify related orders, and document baseline findings','Implement priority safety measures and reassess response within an appropriate timeframe','Escalate worsening findings using SBAR and document patient response']},
  respiration:{sub:['Dyspnea, air hunger, chest tightness, or fatigue with breathing','Cough, sputum change, inability to clear secretions, or anxiety about breathing','Reports orthopnea, activity intolerance, or poor sleep from respiratory symptoms'],obj:['Low SpO2 or abnormal ABG/ETCO2','Tachypnea, accessory muscle use, cyanosis, retractions, or altered mentation','Abnormal breath sounds, weak cough, retained secretions, or high oxygen requirement'],goals:['Maintain ordered oxygenation target and patent airway','Show decreased work of breathing and stable mentation','Demonstrate effective breathing/coughing technique before handoff'],ints:['Assess RR, effort, lung sounds, SpO2, color, cough, and mental status frequently','Position for lung expansion; apply oxygen, suction, nebulizer, or airway support as ordered','Escalate falling saturation, exhaustion, silent chest, cyanosis, or altered level of consciousness']},
  foodfluid:{sub:['Thirst, nausea, vomiting, diarrhea, poor intake, dysphagia, or appetite change','Weakness, dizziness, cramps, palpitations, or confusion','Food insecurity, cultural diet concern, breastfeeding difficulty, or medication access barrier'],obj:['Abnormal intake/output, weight change, edema, dry mucosa, poor skin turgor, or concentrated urine','Abnormal glucose, electrolytes, albumin, ketones, renal or liver indicators','Aspiration signs, poor latch, swallowing difficulty, or enteral/parenteral nutrition device present'],goals:['Maintain ordered fluid, nutrition, glucose, and electrolyte targets','Show stable vital signs, urine output, weight trend, and mental status','Patient/caregiver demonstrates diet, hydration, medication, or feeding plan using teach-back'],ints:['Trend I/O, daily weight, labs, glucose checks, swallowing/feeding tolerance, and hydration findings','Administer ordered fluids, nutrition support, antiemetics, replacements, or glucose treatment safely','Escalate critical labs, persistent losses, aspiration signs, hypoglycemia/hyperglycemia, or poor urine output']},
  pain:{sub:['Rates pain, discomfort, pressure, cramping, burning, or labor pain','Describes location, quality, onset, triggers, and relief measures','Reports anxiety, poor sleep, limited movement, or fear related to pain'],obj:['Guarding, grimacing, diaphoresis, restlessness, crying, or rigid posture','Tachycardia, hypertension, shallow breathing, or reduced mobility','Wound, contraction pattern, inflammation, procedure, or injury consistent with pain source'],goals:['Pain is at or below patient-stated acceptable level after intervention','Patient demonstrates improved rest, breathing, mobility, or participation in care','Cause of new, severe, or changing pain is evaluated promptly'],ints:['Assess pain using PQRST, scale, functional impact, and safety risks before and after intervention','Administer ordered analgesics and nonpharmacologic measures; reassess within expected onset time','Report unrelieved, new, radiating, or unstable pain findings using SBAR']},
  safety:{sub:['Reports dizziness, weakness, suicidal thoughts, fear, itching, exposure, or unsafe environment','States history of falls, allergy, violence, self-harm, infection exposure, or poor wound care','Caregiver describes behavior change, wandering, withdrawal, or inability to follow safety plan'],obj:['Fever, wound change, drainage, rash, bleeding, bruising, poor skin integrity, or abnormal WBC/coagulation','Confusion, impulsivity, gait instability, restraints risk, equipment hazards, or high-risk medication use','Isolation need, invasive line, surgical site, pregnancy/newborn risk, or documented safety alert'],goals:['Remain free from preventable injury, infection, bleeding, or self/other harm','Risk factors are reduced and safety precautions remain in place through handoff','Patient/caregiver verbalizes warning signs and when to seek help'],ints:['Apply fall, infection, bleeding, skin, allergy, suicide, or violence precautions matched to the diagnosis','Inspect environment, skin/wound/line, mental status, and medication risks each round','Escalate fever, sepsis signs, active bleeding, allergic reaction, self-harm risk, or unsafe behavior immediately']},
  circulation:{sub:['Chest pain, palpitations, headache, dizziness, syncope, weakness, or visual change','Reports coldness, numbness, edema, activity intolerance, or neurologic change','States medication nonadherence, bleeding symptoms, or cardiovascular history'],obj:['Abnormal BP/HR/rhythm, capillary refill, pulses, edema, skin temperature/color, or neuro status','Abnormal ECG, troponin, lactate, coagulation, hemoglobin, or perfusion indicators','Urine output decline, altered mentation, mottling, or signs of shock'],goals:['Maintain adequate perfusion with stable vital signs, mentation, pulses, and urine output','Identify and respond to deterioration before organ injury progresses','Patient verbalizes priority cardiovascular precautions and medications'],ints:['Trend vital signs, rhythm, pulses, perfusion, neuro checks, pain, and urine output','Administer ordered fluids, vasoactive/cardiac meds, anticoagulants, or antihypertensives with required checks','Escalate chest pain, stroke signs, shock, uncontrolled BP, dysrhythmia, bleeding, or worsening perfusion']},
  elimination:{sub:['Reports constipation, diarrhea, urgency, retention, dysuria, incontinence, or abdominal discomfort','States change in usual bowel/urinary pattern or embarrassment limiting reporting','Reports poor intake, medication effects, pain, or mobility barriers'],obj:['Abdominal distention, hypo/hyperactive bowel sounds, bladder distention, abnormal stool/urine','I/O imbalance, abnormal urinalysis, renal labs, stool findings, or catheter/device issue','Skin breakdown, odor, fever, dehydration signs, or mobility/toileting limitation'],goals:['Restore or maintain safe bowel/urinary elimination pattern','Prevent dehydration, infection, skin breakdown, and retention-related complications','Patient/caregiver explains toileting, hydration, diet, medication, or catheter plan'],ints:['Assess bowel/urinary pattern, abdomen/bladder, I/O, stool/urine characteristics, and skin integrity','Implement toileting schedule, hydration/diet measures, ordered meds, catheter care, or specimen collection','Escalate retention, anuria/oliguria, blood, severe diarrhea, fever, pain, or worsening renal indicators']},
  activity:{sub:['Reports fatigue, insomnia, weakness, poor endurance, dizziness, or difficulty with ADLs','States fear of activity, sleep disruption, mobility limits, or inability to transfer safely','Reports lack of recreation, deconditioning, or need for assistive device'],obj:['Limited ROM, impaired gait, low endurance, abnormal response to activity, or need for assistance','Sleep deprivation signs, daytime somnolence, poor concentration, or restlessness','Fall risk factors, immobility complications, pressure risk, or assistive equipment need'],goals:['Complete required activity/ADL with safe assist level and stable physiologic response','Improve rest/activity balance with reduced fatigue and no preventable immobility complication','Patient demonstrates energy conservation, sleep hygiene, or transfer technique'],ints:['Assess baseline mobility, ADLs, sleep/rest, fatigue, pain, vital response, and fall risk','Cluster care, pace activity, support sleep, use assistive devices, and progress activity gradually','Stop activity and report chest pain, severe dyspnea, syncope, unsafe gait, or abnormal vital response']},
  ego:{sub:['Verbalizes fear, anxiety, hopelessness, grief, spiritual distress, low self-worth, or decisional conflict','Reports trauma memories, loss of control, moral concern, or inability to cope','States cultural, family, role, body image, or dignity concern'],obj:['Tearfulness, agitation, withdrawal, panic signs, poor concentration, or labile affect','Unsafe coping behavior, refusal of care, impaired decision-making, or altered interaction','Sleep/appetite change, poor adherence, self-harm cues, or family conflict'],goals:['Patient expresses feelings and identifies at least one coping support or next step','Safety is maintained while anxiety, grief, distress, or decision conflict is addressed','Patient participates in care decisions consistent with values and readiness'],ints:['Use therapeutic communication, privacy, cultural humility, and short clear explanations','Assess coping, supports, suicide/self-harm risk, spiritual needs, and readiness for information','Offer choices, involve support persons/chaplain/mental health resources, and escalate safety concerns']},
  social:{sub:['Reports family conflict, caregiver strain, isolation, loneliness, parenting concern, or communication barrier','States lack of support, role strain, immigration stress, or relationship concern','Caregiver reports inability to manage care demands or disrupted family routines'],obj:['Limited eye contact, impaired speech, withdrawal, unsafe family interaction, or unmet care needs','Missed appointments/meds due to support gap, caregiver exhaustion, or communication limitation','Interpreter need, hearing/speech barrier, or absent decision support'],goals:['Patient/family identifies support plan and communicates key needs before handoff','Caregiver strain or role concern is assessed and matched with resources','Communication barriers are reduced for safe care decisions'],ints:['Assess support system, caregiver load, communication needs, culture, health literacy, and safety at home','Use interpreter/communication aids; include family or caregiver with consent','Refer to social work, community resources, family conference, or mental health support as indicated']},
  teaching:{sub:['Asks questions, states knowledge gap, or verbalizes readiness to learn','Reports risky health behavior, poor adherence, or difficulty accessing care','Caregiver/student identifies uncertainty about diagnosis, meds, warning signs, or follow-up'],obj:['Incorrect return demonstration, medication error risk, missed care, or repeated exacerbation','Low health literacy indicators, developmental delay risk, or inconsistent self-management data','Discharge need, new diagnosis, new medication/device, or community health concern'],goals:['Patient/caregiver explains diagnosis, priority action, warning signs, and follow-up using teach-back','Learner demonstrates safe medication, device, wound, diet, or symptom-management skill','Barriers to adherence are identified with a feasible Philippine-context resource plan'],ints:['Assess readiness, literacy, language, beliefs, resources, and learning priorities','Teach in small chunks using demonstration, written cues, and teach-back','Address barriers such as cost, access, transport, family support, and community referral needs']}
};
const CARE_GUIDE_OVERRIDES={
  'Acute Pain':{goals:['Patient reports pain at acceptable level within ordered reassessment window','Patient demonstrates improved comfort, breathing, rest, or movement after intervention','New or worsening pain is evaluated and escalated promptly'],ints:['Assess pain using PQRST, scale, functional impact, sedation/respiratory risk, and prior response','Administer ordered analgesic and nonpharmacologic measures; reassess at peak effect','Notify provider for unrelieved pain, new chest/abdominal pain, neurologic change, or unstable vitals']},
  'Deficient Fluid Volume':{goals:['Urine output, mental status, mucous membranes, and vital signs trend toward hydration target','Patient tolerates ordered oral or IV replacement without worsening symptoms','Ongoing fluid losses are identified and controlled or escalated'],ints:['Track strict I/O, daily weight, mucous membranes, skin turgor, orthostatics, and urine concentration','Administer ordered fluids/electrolytes and antiemetics safely; reassess after each bolus or replacement','Report hypotension, tachycardia, poor urine output, ongoing vomiting/diarrhea, or mental-status change']},
  'Risk for Infection':{goals:['Patient remains free from fever, purulent drainage, new respiratory/urinary symptoms, or sepsis signs','Line, wound, skin, and device sites remain clean and monitored','Patient/caregiver states infection warning signs and prevention steps'],ints:['Perform hand hygiene, aseptic technique, ordered isolation, line/wound care, and device necessity checks','Trend temperature, WBC, wound/line findings, respiratory/urinary symptoms, and glucose when relevant','Escalate fever, worsening redness/drainage, hypotension, altered mentation, or suspected sepsis']},
  'Impaired Gas Exchange':{goals:['SpO2/ABG remains within ordered target with stable mentation','Work of breathing decreases and patient tolerates activity or rest position','Patient demonstrates breathing/coughing strategy appropriate to condition'],ints:['Assess SpO2, RR, effort, lung sounds, color, mentation, ABG/ETCO2, and response to oxygen','Position upright, administer oxygen/respiratory meds as ordered, and coordinate respiratory therapy','Escalate rising oxygen need, cyanosis, exhaustion, silent chest, or altered level of consciousness']},
  'Risk for Falls':{goals:['Patient remains free from fall or injury during the encounter','Fall-risk factors are reduced before mobility or medication administration','Patient uses call light and prescribed assist level consistently'],ints:['Keep bed low/locked, call light within reach, non-skid footwear on, clutter cleared, and alarms/assistance as indicated','Assess gait, orthostatics, dizziness, cognition, toileting needs, lines/tubes, and sedating medications','Teach patient to call before standing and reassess after antihypertensive, opioid, sedative, or diuretic doses']},
  'Risk for Electrolyte Imbalance':{goals:['Electrolytes remain within ordered range without dysrhythmia, seizure, weakness, or mental-status change','Replacement therapy is given safely with required monitoring','Critical values are recognized and escalated promptly'],ints:['Trend BMP/Mg/Ca/glucose, ECG/telemetry, neuromuscular status, GI losses, and renal function','Verify replacement dose, route, dilution, pump rate, compatibility, and lab prerequisites','Escalate critical potassium, sodium, calcium, magnesium, glucose, dysrhythmia, seizure, or worsening weakness']}
};
function careGuideFor(dx){const base=CARE_GUIDE_DOMAIN_TEXT[careGuideDomain(dx)]||CARE_GUIDE_DOMAIN_TEXT.general;const o=CARE_GUIDE_OVERRIDES[dx]||{};return{dx,assessment:o.assessment||base,goals:o.goals||base.goals,interventions:o.ints||base.ints};}
const NURSING_DIAGNOSIS_LIBRARY=POCKET_GUIDE_DIAGNOSIS_NAMES.map(careGuideFor);
window.NURSING_DIAGNOSIS_LIBRARY=NURSING_DIAGNOSIS_LIBRARY;
function carePlanEmpty(){return{assessment:'',dx:'',goal:'',interventions:'',evaluation:''};}
function diagnosisOptions(cur){return '<option value="">Choose Pocket Guide diagnosis</option>'+NURSING_DIAGNOSIS_LIBRARY.map(d=>`<option value="${esc(d.dx)}" ${d.dx===cur?'selected':''}>${esc(d.dx)}</option>`).join('');}
function carePlanSelect(id,items,cur,label){return`<select id="${id}"><option value="">${esc(label)}</option>${(items||[]).map(x=>`<option value="${esc(x)}" ${x===cur?'selected':''}>${esc(x)}</option>`).join('')}</select>`;}
function carePlanAssessmentSelect(i,tpl,cur){let cueHtml='<option value="">Add subjective/objective cue</option>';if(tpl){cueHtml+='<optgroup label="Subjective cues">'+(tpl.assessment.subjective||[]).map(x=>`<option value="Subjective: ${esc(x)}">Subjective: ${esc(x)}</option>`).join('')+'</optgroup>';cueHtml+='<optgroup label="Objective cues">'+(tpl.assessment.objective||[]).map(x=>`<option value="Objective: ${esc(x)}">Objective: ${esc(x)}</option>`).join('')+'</optgroup>';}return`<select data-cp-cue="${i}">${cueHtml}</select><textarea data-cp="assessment" data-cp-index="${i}" placeholder="Categorized assessment cues: Subjective... Objective...">${esc(cur||'')}</textarea>`;}
function rCarePlan(){const rows=currentPatient().carePlan.map((c,i)=>{const tpl=NURSING_DIAGNOSIS_LIBRARY.find(d=>d.dx===c.dx);return`<tr><td>${carePlanAssessmentSelect(i,tpl,c.assessment||c.cues||'')}</td><td><select data-cp-template="${i}">${diagnosisOptions(c.dx)}</select></td><td>${carePlanSelect(`cp-goal-${i}`,tpl?.goals,c.goal,'Choose goal / priority')}<textarea data-cp="goal" data-cp-index="${i}" placeholder="Goal / priority">${esc(c.goal||'')}</textarea></td><td>${carePlanSelect(`cp-int-${i}`,tpl?.interventions,c.interventions,'Choose intervention')}<textarea data-cp="interventions" data-cp-index="${i}" placeholder="Interventions">${esc(c.interventions||'')}</textarea></td><td><textarea data-cp="evaluation" data-cp-index="${i}" placeholder="Student evaluation">${esc(c.evaluation||'')}</textarea></td><td><button class="btn small danger" data-del-cp="${i}">Delete</button></td></tr>`}).join('');return section('Nursing care plan',`<div class="notice info"><span class="mark">GUIDE</span><span>Diagnoses are drawn from the Nurse's Pocket Guide NANDA-I listing. Select a diagnosis to populate categorized assessment cues, goals/priorities, and interventions; write your own evaluation.</span></div>`+table(['Assessment cues','Diagnosis','Goal / Priority','Interventions','Evaluation',''],rows)+'<div class="actions"><button class="btn primary" id="add-careplan">Add row</button><button class="btn navy" id="save-careplan">Save care plan</button></div>');}
...
function rReport(){
  ...
  const existingNames = new Set((p.meds||[]).map(m=>(m.name||'').toLowerCase().trim()));
  const syncedMeds   = (p.scenarioId === 'scratch' ? (p.orders||[]) : [])
  ...
  // ── Care plan ─────────────────────────────────────────────────────────────
  const carePlanRows = (p.carePlan||[]).filter(c=>c.dx||c.assessment).map(c=>`<tr>
    <td style="font-size:11px;">${lines(c.assessment||c.cues||'')}</td>
    <td>${esc(c.dx)}</td>
  ...
  </tr>`).join('') || '<tr><td colspan="5" style="color:var(--subtle);">No care plan documented.</td></tr>';
  ...
  return `<div class="print-report">
    ...
    ${section('Printable chart report',`
      ...
      <div class="actions" style="justify-content:flex-start;">
        <button class="btn primary" onclick="printWithMode('report')">🖨️ ${tr('printReport')}</button>
      </div>
    `)}
    ...
    ${section('Nursing care plan',
      table(['Assessment','Nursing Diagnosis','Goal','Interventions','Evaluation'], carePlanRows)
    )}
...
function rDashboard(){
  if(!cloud.profile)return section('Analytics dashboard','<div class="notice info"><span class="mark">LOADING</span><span>Profile loading. Sign out and back in if this persists.</span></div>');
  if(!canAccessAnalytics())return section('Analytics dashboard','<div class="notice danger"><span class="mark">LOCKED</span><span>Analytics access is restricted to approved HCT Institute accounts only.</span></div>');
...
function rStatusBoard(){
  ...
  if(!canAccessAnalytics())return section('Status board','<div class="notice danger"><span class="mark">LOCKED</span><span>Analytics access is restricted to approved HCT Institute accounts only.</span></div>');
...
function bindEvents(){
  ...
  document.getElementById('print-report-btn')?.addEventListener('click',()=>{state.tab='report';persist({cloud:false});render();setTimeout(()=>printWithMode('report'),120);});
  bindCloudEvents();
  ...
  document.querySelectorAll('[data-nav-toggle]').forEach(btn=>btn.onclick=e=>{e.stopPropagation();const id=btn.dataset.navToggle;const grp=document.querySelector(`[data-nav-section="${id}"]`);if(grp){grp.classList.toggle('open');navOpen[id]=grp.classList.contains('open');}});
...
function exportJsonBackup(){
  const payload={app:'HCT EHR Simulation',version:3,exportedAt:new Date().toISOString(),state};
...
function importJsonBackup(e){
  ...
  reader.onload=()=>{
    try{
      ...
      if(!incoming||!Array.isArray(incoming.patients))throw new Error('Invalid HCT EHR JSON backup');
      state={...incoming,settings:incoming.settings||settings(),submitted:incoming.submitted||{}};
      if(!state.activeId&&state.patients[0])state.activeId=state.patients[0].id;
      persist();
      render();
...
function bindTabEvents(){
  ...
  document.querySelectorAll('[data-med-act]').forEach(b=>b.onclick=()=>{
    ...
    m.status=b.dataset.medAct;if(!m.time)m.time=nowTime();persist();render();
  });
  ...
  document.getElementById('add-careplan')?.addEventListener('click',()=>{p.carePlan.push(carePlanEmpty());persist();render();});
  document.querySelectorAll('[data-cp-template]').forEach(sel=>sel.addEventListener('change',()=>{
    const i=Number(sel.dataset.cpTemplate),tpl=NURSING_DIAGNOSIS_LIBRARY.find(d=>d.dx===sel.value);
    if(!tpl||!p.carePlan[i])return;
    p.carePlan[i]={...p.carePlan[i],dx:tpl.dx,assessment:'',goal:'',interventions:'',evaluation:''};
    persist();render();
  }));
  document.querySelectorAll('[data-cp-cue]').forEach(sel=>sel.addEventListener('change',()=>{const i=Number(sel.dataset.cpCue);if(!sel.value||!p.carePlan[i])return;const existing=p.carePlan[i].assessment||'';p.carePlan[i].assessment=existing?existing+'\n'+sel.value:sel.value;persist();render();}));
  document.querySelectorAll('[id^="cp-goal-"]').forEach(sel=>sel.addEventListener('change',()=>{const i=Number(sel.id.replace('cp-goal-',''));if(p.carePlan[i])p.carePlan[i].goal=sel.value;persist();render();}));
  document.querySelectorAll('[id^="cp-int-"]').forEach(sel=>sel.addEventListener('change',()=>{const i=Number(sel.id.replace('cp-int-',''));if(p.carePlan[i])p.carePlan[i].interventions=sel.value;persist();render();}));
  ...
  document.querySelectorAll('[data-load-scenario]').forEach(b=>b.onclick=()=>{const sc=scenarioLibrary().find(s=>s.id===b.dataset.loadScenario);if(!sc)return;const next=clone(sc.patient);next.id=uid(sc.id);state.patients.push(next);state.activeId=next.id;state.tab='summary';persist();render();});
  document.getElementById('fs-template')?.addEventListener('click',()=>{document.getElementById('fs-name').value='PNLE community-acquired pneumonia with sepsis risk';document.getElementById('fs-focus').value='Adult Health / Emergency';document.getElementById('fs-skills').value='Airway, Sepsis screening, Antibiotics, SBAR';document.getElementById('fs-first').value='Jose';document.getElementById('fs-last').value='Santos';document.getElementById('fs-age').value='67';document.getElementById('fs-sex').value='Male';document.getElementById('fs-location').value='ED Bay 2';document.getElementById('fs-diagnosis').value='Community-acquired pneumonia with sepsis risk';document.getElementById('fs-acuity').value='Urgent';document.getElementById('fs-cc').value='Fever, productive cough, and shortness of breath';document.getElementById('fs-hpi').value='Three days of fever and cough with yellow sputum. Family reports increasing weakness and mild confusion today.';document.getElementById('fs-history').value='Hypertension and type 2 diabetes. Lives with daughter in Quezon City. No known drug allergies.';document.getElementById('fs-vitals').value='HR 118, BP 96/58, RR 26, T 38.8 C, SpO2 90%, pain 2';document.getElementById('fs-orders').value='Nursing | Sepsis screen and notify provider | Document time zero and focused assessment | STAT\nLab | CBC, CMP, lactate, blood cultures x2 | Draw cultures before antibiotics if this does not delay care | STAT\nMedication | Ceftriaxone 1 g IV | Verify allergy history | STAT\nRespiratory | Oxygen via nasal cannula | Titrate to ordered SpO2 target | Active';document.getElementById('fs-labs').value='09:15 | WBC | 17.8 K/uL | 4.5-11 | High | Infection pattern\n09:15 | Lactate | 2.6 mmol/L | 0.5-2.0 | High | Repeat per protocol\n09:20 | CXR | Right lower lobe infiltrate | Normal | High | Consistent with pneumonia';document.getElementById('fs-expected').value='Apply oxygen and reassess respiratory status\nRecognize sepsis risk and escalate promptly\nObtain cultures before antibiotics when feasible\nAdminister ordered antibiotics safely\nDocument SBAR with vital signs and lactate';document.getElementById('fs-prompts').value='Which assessment cue makes this patient a sepsis risk?\nWhy should blood cultures be obtained before antibiotics when feasible?\nWhat nursing action has the highest priority if oxygen saturation continues to fall?';document.getElementById('fs-debrief').value='What cues did you prioritize first and why?\nHow did your intervention sequence protect patient safety?\nWhat would you report in SBAR before transfer or admission?';});
  document.getElementById('fs-save')?.addEventListener('click',()=>{const name=val('fs-name');const diagnosis=val('fs-diagnosis');if(!name||!diagnosis)return toast('Scenario name and diagnosis required');const splitLines=id=>val(id).split('\n').map(x=>x.trim()).filter(Boolean);const rows=id=>splitLines(id).map(line=>line.split('|').map(x=>x.trim()));const id=uid('faculty');const v=val('fs-vitals');const vital={time:nowTime(),hr:(v.match(/HR\s*([0-9]+)/i)||[])[1]||'',bps:(v.match(/BP\s*([0-9]+)\s*\/\s*([0-9]+)/i)||[])[1]||'',bpd:(v.match(/BP\s*([0-9]+)\s*\/\s*([0-9]+)/i)||[])[2]||'',rr:(v.match(/RR\s*([0-9]+)/i)||[])[1]||'',temp:(v.match(/T\s*([0-9.]+)/i)||[])[1]||'',spo2:(v.match(/SpO2\s*([0-9]+)/i)||[])[1]||'',pain:(v.match(/pain\s*([0-9]+)/i)||[])[1]||'',note:v,by:'Faculty scenario'};const prompts=splitLines('fs-prompts');REASONING_PROMPTS[id]=prompts.length?prompts:REASONING_PROMPTS.scratch;const expected=splitLines('fs-expected');const debrief=splitLines('fs-debrief');const patient=patientTemplate({id:id+'-patient',scenarioId:id,firstName:val('fs-first')||'Patient',lastName:val('fs-last')||'Scenario',age:val('fs-age'),sex:val('fs-sex'),mrn:val('fs-mrn')||randomMrn(),location:val('fs-location')||'Simulation room',diagnosis,acuity:val('fs-acuity'),chiefComplaint:val('fs-cc')||diagnosis,hpi:val('fs-hpi'),background:val('fs-history'),pastHistory:val('fs-history'),objectives:expected.length?expected:['Prioritize assessment cues.','Select appropriate nursing diagnosis.','Implement safe interventions.','Use SBAR for escalation.'],orders:rows('fs-orders'),labs:rows('fs-labs'),vitals:[vital]});patient.expectedInterventions=expected;patient.debriefingQuestions=debrief;const sc={id,name,focus:val('fs-focus')||'Faculty-created PNLE case',level:val('fs-level')||'Intermediate',summary:[val('fs-cc'),diagnosis].filter(Boolean).join(' - '),skills:(val('fs-skills')||'PNLE reasoning').split(',').map(x=>x.trim()).filter(Boolean),patient,scenarioObject:{patientDemographics:{firstName:patient.firstName,lastName:patient.lastName,age:patient.age,sex:patient.sex,mrn:patient.mrn,location:patient.location},chiefComplaint:patient.chiefComplaint,history:{hpi:patient.hpi,pastHistory:patient.pastHistory,background:patient.background},vitals:patient.vitals,orders:patient.orders,labs:patient.labs,expectedNursingInterventions:expected,pnleReasoningPrompts:prompts,debriefingQuestions:debrief}};state.customScenarios=state.customScenarios||[];state.customScenarios.unshift(sc);persist();toast('Scenario saved to Sample Scenarios');render();});
...
(function handleUrlPatient(){
  try {
    ...
    if (!pid) return;
    const sc = scenarioLibrary().find(s => s.id === pid);
    if (!sc) return;
